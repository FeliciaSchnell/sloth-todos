<template>
  <div class="taskList">
    ...
    <div class="listItem" v-for="(item, index) in data" :key="index">
      <input class="checkbox" type="checkbox" v-model="item.completed" :checked="item.completed" @click="$emit('taskUpdate')">
      <input class="taskName" type="text" v-model="item.name" placeholder="Title" @keypress="$emit('taskUpdate')">
